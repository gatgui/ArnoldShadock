...
node_initialize
{
   AiNodeSetLocalData(node, AiMalloc(sizeof(CookTorranceBrdfData)));
}

node_update
{
   CookTorranceBrdfData *data = (CookTorranceBrdfData*) AiNodeGetLocalData(node);
...
shader_evaluate
{
   CookTorranceBrdfData *node_data = (CookTorranceBrdfData*) AiNodeGetLocalData(node);
