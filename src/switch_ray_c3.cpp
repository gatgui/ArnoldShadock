...
node_parameters
{
   AiParameterRGB("camera", 0.0f, 0.0f, 0.0f);
   AiParameterBool("camera_use_default", true);
   AiParameterRGB("shadow", 0.0f, 0.0f, 0.0f);
   AiParameterBool("shadow_use_default", true);
   AiParameterRGB("reflected", 0.0f, 0.0f, 0.0f);
...
shader_evaluate
{
   ...
   {
      sg->out.RGB = AiShaderEvalParamRGB((data->camera_linked || !data->camera_use_default) ? p_camera : p_default);
   }
   else if ((sg->Rt & AI_RAY_SHADOW) != 0)
   {
      sg->out.RGB = AiShaderEvalParamRGB((data->shadow_linked || !data->shadow_use_default) ? p_shadow : p_default);
   }
   else if ((sg->Rt & AI_RAY_REFLECTED) != 0)
   {
      sg->out.RGB = AiShaderEvalParamRGB((data->reflected_linked || !data->reflected_use_default) ? p_reflected : p_default);
   }
   ...
   {
      sg->out.RGB = AiShaderEvalParamRGB((data->glossy_linked || !data->glossy_use_default) ? p_glossy : p_default);
   }
   else
   {
